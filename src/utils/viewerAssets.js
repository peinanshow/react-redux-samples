...
export function createMarkup(rawData, resourceHandlerEndpoint, pageNumber = 1) {
  ...
  const banners = documentContent.querySelectorAll('div.page+div:not(.page)');
  if (banners.length) {
    banners[0].style.left = 'auto';
    banners[0].style.top = 'auto';
    banners[0].style.bottom = '0';
    banners[0].style.width = '100%';
    banners[0].style.textAlign = 'center';
    banners[0].style.padding = '20px 50px';
    for (let key = 1; key < banners.length; key++) {
...
export function downloadDocument(documentUrl, useBlank = true) {
  ...
  if (useBlank) {
    const target = window.open(exportUrl, '_blank');
